...
public class EmployeeValidator {
    ...
    // ── Regex patterns (sourced from actual CSV data) ─────────────────────────

    /** Employee number: 1-6 digits, e.g. 10001 */
    private static final String EMP_NUM_PATTERN      = "\\d{1,6}";

    /** Name: letters, spaces, periods, hyphens, apostrophes, Roman numerals */
    ...
    /** Phone (PH local): NNN-NNN-NNN  e.g. 966-860-270 */
    private static final String PHONE_PATTERN        = "\\d{3}-\\d{3}-\\d{3}";

    /** SSS: NN-NNNNNNN-N  e.g. 44-4506057-3 */
    private static final String SSS_PATTERN          = "\\d{2}-\\d{7}-\\d";

    /** PhilHealth: 12 digits, no dashes  e.g. 820126853951 */
    private static final String PHILHEALTH_PATTERN   = "\\d{12}";

    /** TIN: NNN-NNN-NNN-NNN  e.g. 442-605-657-000 */
    private static final String TIN_PATTERN          = "\\d{3}-\\d{3}-\\d{3}-\\d{3}";

    /** Pag-Ibig: 12 digits, no dashes  e.g. 691295330870 */
    private static final String PAGIBIG_PATTERN      = "\\d{12}";

    /** Numeric: positive integer or decimal, e.g. 90000 or 535.71 */
    ...
    /** Address allows free text — only blank check. */
    ...
    /**
     * Validates all employee fields.  Returns an empty list when everything
     * is valid, or a list of error messages to display to the HR user.
     *
     * @param checkEmpNumDuplicate  when true, also checks that the employee
     *                              number does not already exist in existingIds.
     */
    public static List<String> validateAll(
            ...
            boolean checkEmpNumDuplicate,
            java.util.Set<String> existingEmpNumbers) {
